...
$display_player_name = '';
$user_vnd = 0;
$user_avatar = '/images/avatar/0.png'; // Avatar mặc định
...
if (isset($_SESSION['username'])) {
    ...
    if ($stmt_user_data) {
        ...
        if ($user_row = $result_user_data->fetch_assoc()) {
            ...
            $is_admin_for_avatar = $user_row['is_admin'] ?? 0;
            ...
            if ($is_admin_for_avatar == 1) {
                $user_avatar = "/images/avatar/6101.gif";
            } else {
                if ($player_head_for_avatar > 0) {
                    ...
                    if ($player_gender_for_avatar == 0) {
                        ...
                        $user_avatar = "/images/avatar/0.png"; // Default for non-admin if head/gender is unexpected
                    }
                }
            }
...
if ($result_pinned) {
    while ($row = $result_pinned->fetch_assoc()) {
        $row['avatar_url'] = get_post_avatar_url($row['is_admin'] ?? 0, $row['gender'] ?? 0, $row['head'] ?? 0, $row['ghimbai'] ?? 0);
        $pinned_posts[] = $row;
    }
    $result_pinned->free();
} else {
