...
$_is_logged_in = false;
$_username = '';
$_user_avatar = '/images/avatar/0.png'; // Đường dẫn avatar mặc định
...
if (isset($_SESSION['username']) && isset($conn)) { // Đảm bảo $conn đã có
    ...
    if ($stmt_user_data) {
        ...
        if ($user_row) {
            ...
            $_admin_logged_in = $user_row['is_admin'] ?? 0;
            $tichdiem_logged_in = $user_row['tichdiem'] ?? 0;
            $_coin = $user_row['cash'] ?? 0;
            ...
            if ($_admin_logged_in == 1) {
                ...
                if ($logged_in_player_gender == 1) {
                    $_user_avatar = "/images/avatar/10.png"; // Admin nam
                } elseif ($logged_in_player_gender == 2) {
                    $_user_avatar = "/images/avatar/11.png"; // Admin nữ
                } else {
                    $_user_avatar = "/images/avatar/12.png"; // Admin khác
                }
            } else {
                ...
                if ($player_head > 0) { // Đảm bảo head có giá trị hợp lệ
                    ...
                    if ($logged_in_player_gender == 1) {
                        $_user_avatar = "/images/avatar/1.png"; // User nam mặc định
                    } elseif ($logged_in_player_gender == 2) {
                        $_user_avatar = "/images/avatar/2.png"; // User nữ mặc định
                    } else {
                        $_user_avatar = "/images/avatar/0.png"; // User khác mặc định
                    }
                }
            }
