...
// ============================================
// WORD FILTER - Danh sách t? nh?y c?m 18+
// ============================================
$banned_words = [
    // Vietnamese 18+ words
    'd?', 'd?t', 'dcm', 'dcm', 'dmm', 'dmm', 'clgt', 'c?c', 'bu?i', 'l?n', 'déo',
    'vãi', 'vl', 'vcl', 'vkl', 'di', 'cave', 'phò', 'dâm', 's?c', 'th? dâm',
    'n?ng', 'ch?ch', 'fuck', 'shit', 'dick', 'pussy', 'bitch', 'asshole', 'nigger',
    'sex', 'porn', 'xxx', 'nude', 'nudes', 'onlyfans', 'blowjob', 'handjob',
    'con m?', 'con di', 'd? chó', 'th?ng chó', 'con chó', 'm? mày',
    'hack', 'cheat', 'bug vàng', 'dupe', 'l?a d?o', 'scam',
    'cc', 'cl', 'dm', 'dm', 'vcc', 'wtf', 'stfu', 'dkm', 'dkm',
    'kh?n n?n', 'm?t l?n', 'c?t', 'dái', '?a'
];
...
// ============================================
// RANK SYSTEM - Check quy?n dang bài
// ============================================
$user_tongnap = 0;
$user_is_admin = 0;
$_alert = '';
...
if (!$can_post) {
    $blocked_reason = 'B?n c?n n?p ít nh?t 1 l?n d? du?c quy?n dang bài trên di?n dàn.';
}
...
if ($can_post && $user_rank !== 'admin') {
    ...
    if ($posts_today >= $max_posts_per_day) {
        $can_post = false;
        $blocked_reason = "B?n dã d?t gi?i h?n $max_posts_per_day bài/ngày. Quay l?i ngày mai nhé!";
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $can_post) {
    ...
    if ($bad_title !== false) {
        $_alert = "<div class='message error'>? Tiêu d? ch?a t? ng? không phù h?p: \"$bad_title\". Vui lòng ch?nh s?a l?i.</div>";
    } elseif ($bad_content !== false) {
        $_alert = "<div class='message error'>? N?i dung ch?a t? ng? không phù h?p: \"$bad_content\". Vui lòng ch?nh s?a l?i.</div>";
    } elseif (strlen($tieude) < 5 || strlen(trim(strip_tags($noidung))) < 5) {
        $_alert = "<div class='message error'>Tiêu d? và n?i dung ph?i có ít nh?t 5 ký t?!</div>";
    } else {
        if (!isset($_username)) {
            $_alert = "<div class='message error'>L?i: Không th? xác d?nh tên ngu?i dùng.</div>";
        } else {
            ...
            if ($stmt_player_name) {
                ...
                if ($stmt_insert_post) {
                    ...
                    if ($stmt_insert_post->execute()) {
                        ...
                        $_SESSION['alert_message'] = "<div class='message success'>Bài vi?t dã du?c dang thành công.</div>";
                        header("Location: /forum.php");
                        exit();
                    } else {
                        $_alert = "<div class='message error'>L?i khi dang bài vi?t: " . $stmt_insert_post->error . "</div>";
                    }
...
<head>
    ...
    <title>Ðang bài vi?t m?i - Di?n Ðàn</title>
...
<body>
    ...
    <div class="post-container">
        <a href="/forum.php" class="back-link">
            ...
            H?y và quay l?i Di?n Ðàn
        </a>

        <div class="apple-post-card">
            <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom: 16px;">
                <h2 style="margin:0; font-size:28px; font-weight:700;">T?o Bài Vi?t M?i</h2>
            ...
            <?php if (!$can_post): ?>
                <div class="blocked-box">
                    ...
                    <h3>Chua d? quy?n dang bài</h3>
                    <p><?php echo $blocked_reason; ?></p>
                    <?php if ($user_rank === 'none'): ?>
                    <div class="rank-info" style="margin-top: 20px; text-align: left;">
                        <strong>?? H? th?ng c?p b?c Di?n Ðàn:</strong><br>
                        ?? <strong>Tân Th?</strong> (n?p > 0?): 1 bài/ngày, text only<br>
                        ?? <strong>Chi?n Si</strong> (n?p = 100K): 1 bài/ngày, dang kèm ?nh, 5 cmt/ngày<br>
                        ?? <strong>Cao Th?</strong> (n?p = 500K): 3 bài/ngày, chat vô h?n trong topic mình
                    </div>
                    <?php endif; ?>
            ...
            <?php else: ?>
                <div class="rank-info">
                    ...
                    <?php if (!$can_post_image): ?> • ?? Chua du?c dang ?nh (c?n Chi?n Si tr? lên)<?php endif; ?>
                </div>
                ...
                <form id="postForm" method="POST" action="">
                    <div style="margin-bottom: 20px;">
                        <label style="display:block; margin-bottom:8px; font-weight:600; color:var(--text-secondary); font-size:14px;">Tiêu d? bài vi?t</label>
                        <input name="tieude" type="text" class="apple-input" placeholder="Nh?p tiêu d? th?t súc tích..." required />
                    </div>
                    ...
                    <div style="display: flex; justify-content: flex-end;">
                        <button type="submit" name="submit" class="btn-pill btn-black" style="padding: 14px 40px; font-size: 15px;">Ðang Bài Ngay</button>
                    </div>
                </form>
            <?php endif; ?>
        </div>
