...
<head>
    ...
    <title>N?p ATM Bank | Chú Bé R?ng Online</title>
    <meta name="description" content="N?p VND qua chuy?n kho?n ngân hàng t? d?ng c?ng ti?n." />
...
<main class="atm-wrap">
    <section class="atm-panel">
        <h1 class="atm-title">N?p Ti?n Tài Kho?n</h1>
        <p class="atm-sub">Thanh toán nhanh chóng và an toàn</p>

        <?php if (!$logged_in_username): ?>
        <div class="login-prompt">
            B?n c?n <a href="/login">dang nh?p</a> d? t?o hóa don n?p ti?n theo tài kho?n c?a b?n.
        </div>
        <?php endif; ?>
        ...
        <div class="atm-grid">
            <div>
                <div class="bank-select">
                    ...
                    <div class="bank-line">S? tài kho?n: <span class="v" id="bank-account"><?php echo htmlspecialchars($acb_account); ?></span>
                        <button class="copy-btn" onclick="copyText('<?php echo htmlspecialchars($acb_account); ?>', this)">Sao chép</button>
                    </div>
                    <div class="bank-line">Ch? tài kho?n: <b><?php echo htmlspecialchars($account_holder); ?></b></div>
                    ...
                    <div class="bank-line" style="font-size:12px;color:#64748b">Nhân v?t: <?php echo htmlspecialchars($player_name); ?> - Tài kho?n: <?php echo htmlspecialchars($logged_in_username); ?></div>
                    <?php endif; ?>
            ...
            <div>
                ...
                <div style="margin-top:12px;color:#6b7280;font-size:13px;line-height:1.6;background:#fff;border:1px dashed #e2e8f0;border-radius:12px;padding:12px;">
                    Ghi dúng n?i dung chuy?n kho?n d? h? th?ng nh?n di?n t? d?ng.<br>
                    Sai n?i dung ho?c n?p ngoài gi?i h?n có th? b? treo x? lý.
                </div>
            </div>
        </div>
        ...
        <button class="create-btn" type="button" onclick="createInvoice()">T?O HÓA ÐON</button>
        ...
        <div class="toolbar">
            ...
            <select id="statusFilter">
                ...
                <option value="pending">Ðang x? lý</option>
                <option value="failed">Th?t b?i</option>
                <option value="ignored">Ðã b? qua</option>
            </select>

            <input class="full" type="datetime-local" id="dateFrom" placeholder="T? ngày">
            <input class="full" type="datetime-local" id="dateTo" placeholder="Ð?n ngày">
            <input class="full" type="text" id="searchFilter" placeholder="Tìm ki?m mã hóa don...">
        </div>
            ...
            <table class="atm-table" id="atmTable">
                ...
                <?php if (!empty($recent_deposits)): ?>
                    <?php foreach ($recent_deposits as $dep):
                        ...
                        elseif ($status === 'ignored') { $status_class = 'st-ignored'; $status_label = 'Ðã b? qua'; }
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="8" class="empty">Chua có giao d?ch n?p ti?n nào.</td>
                    </tr>
                <?php endif; ?>
                </tbody>
            </table>
...
<div class="qr-modal" id="qrModal" onclick="closeQrModal(event)">
    <div class="qr-dialog" role="dialog" aria-modal="true" aria-labelledby="qrModalTitle">
        <div class="qr-head">
            <h3 class="qr-title" id="qrModalTitle">Quét QR d? n?p ti?n</h3>
        ...
        <div class="qr-info" style="margin-bottom:10px">
            ...
            <div>S? tài kho?n: <strong id="qrAccountNumber"><?php echo htmlspecialchars($acb_account); ?></strong></div>
            <div>Ch? tài kho?n: <strong id="qrAccountHolder"><?php echo htmlspecialchars($account_holder); ?></strong></div>
...
function copyText(text, btn) {
    navigator.clipboard.writeText(text).then(() => {
        ...
        btn.textContent = 'Ðã chép';
        btn.classList.add('copied');
        setTimeout(() => {
            btn.textContent = old === 'Ðã chép' ? 'Sao chép' : old;
...
function createInvoice() {
    ...
    if (!amount) {
        alert('Vui lòng nh?p s? ti?n c?n n?p.');
        return;
    }
    if (amount < minDeposit) {
        alert('S? ti?n t?i thi?u là ' + minDeposit.toLocaleString('vi-VN') + ' VND.');
        return;
    }
    if (amount > maxDeposit) {
        alert('S? ti?n t?i da là ' + maxDeposit.toLocaleString('vi-VN') + ' VND.');
        return;
    }
